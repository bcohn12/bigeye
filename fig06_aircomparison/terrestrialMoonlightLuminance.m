...
minvisualrange=0.01; maxvisualrange=100;

pupilValuesAir=linspace(minpupil,maxpupil,20);
...
parfor loop1=1:length(pupilValuesAir)
    ...
    for loop2=1:length(rangeValuesAir)
        ...
        Nspace=(pi/4)^2*(T/r)^2*A^2*Ispace*Dt*eta*(1-exp(-k*len));
        Xch=((T*f_night*A)/(r*d))*X_land*Dt;
        ...
        Ntarget=(pi/4)^2*(T/r)^2*A^2*Itarget*Dt*q*(1-exp(-k*len));
