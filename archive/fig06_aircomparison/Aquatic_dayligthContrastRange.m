function aquaticContrastRange    
    ...
    tol=5e-5; depth=8; r_down=5.8; r_hor=3.8;
    C0Range=linspace(-1,4,20);
    
    a=aAquatic_Sun; b=bAquatic_Sun; 
    Kd=KdAquatic_Sun(:,depth); Kh=KhAquatic_Sun(:,depth);
    Ld=LdAquatic_Sun(:,depth); Lh=LhAquatic_Sun(:,depth);
    Bd=BdAquatic_Sun(depth); Bh=BhAquatic_Sun(depth);
    pAbsorb=pAbsorbAquatic_Sun;
    KdF=@(l) interp1(lambda,Kd,l,'pchip'); KhF=@(l) interp1(lambda,Kh,l,'pchip');
    LdF=@(l) interp1(lambda,Ld,l,'pchip'); LhF=@(l) interp1(lambda,Lh,l,'pchip');
        ...
        for i=1:length(C0Range);
            ...
            while abs(possibleSolnDown-1)>tol
                possibleSolnDown=firingThresh(lambda,...
                     pAbsorb,a,b,Kd,Ld,...
                     r_down,A,X,Dt,q,d,k,len,T,M,R,C0);
            ...
            while abs(possibleSolnHor-1)>tol
                possibleSolnHor=firingThresh(lambda,...
                     pAbsorb,a,b,Kh,Lh,...
                     r_hor,A,X,Dt,q,d,k,len,T,M,R,C0);                
            ...
            Kt_hor(i)=liminalContrst(A,Bh,angularSize_hor(i));
