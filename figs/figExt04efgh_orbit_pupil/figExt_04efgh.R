...
########
## QQ plots, box plot and histogram for relating pupil diameter and orbit length
...
###data
eyedata <- as.data.frame(read.csv("../data/paleo/mean_ol_ed_pd_0709.csv", header=T))
ratio <- (eyedata$pd/eyedata$ol)*100
eyedata[2:4] <- lapply(eyedata[2:4],log10)
...
trait[, 2:4] <- lapply(trait[, 2:4], as.character)
trait[, 2:4] <- lapply(trait[, 2:4], as.numeric)
variables <- trait[,2:4]
